...
from src.recognition import PaddleOCRRecognition, SwinTextSpotterRecognition

class OCRDetector:
    def __init__(self, is_paddle_ocr: bool = True) -> None:
        self.is_paddle_ocr = is_paddle_ocr
        if is_paddle_ocr:
            self.recognition = PaddleOCRRecognition()
        else:
            self.recognition = SwinTextSpotterRecognition()
    ...
    def text_detector(self, image_path, is_local=False):
        ...
        boxes, paddle_texts = self.recognition.find_box(image)
        if not boxes:
            return image, None, None
        viet_texts = self.vietnamese_text(boxes, image)
        results_texts = []
        for i, viet_txt in enumerate(viet_texts):
            if viet_txt["text"] != unidecode.unidecode(viet_txt["text"]) or not self.is_paddle_ocr:
                viet_txt["text"] = unidecode.unidecode(viet_txt["text"])
    ...
    def visualize_ocr(self, image, texts, boxes):
        ...
        for box, text in zip(boxes, texts):
            ...
            draw.text((int(x1), int(y1-h-3)), text["text"], font = font, fill = (255, 255, 255))
